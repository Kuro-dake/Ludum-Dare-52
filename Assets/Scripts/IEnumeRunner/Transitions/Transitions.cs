...
namespace IEnumRunner.Transitions
{
    ...
    public class Value
    {
        public float float_value { get; protected set; }
        public Vector4 vector4_value {get; protected set;}
        ...
        public static Value Lerp(Value origin, Value target, float t) =>
            origin.is_float ? (Value)Mathf.Lerp(origin, target, t) : (Value)Vector4.Lerp(origin, target, t);

        public static Value MoveTowards(Value origin, Value target, float t) =>
            origin.is_float ? (Value)Mathf.MoveTowards(origin, target, t) : (Value)Vector4.MoveTowards(origin, target, t);
